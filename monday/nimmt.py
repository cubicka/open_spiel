...
class NimmtState(object):
  def __init__(self, game):
        self._cur_player = 0
        self._mode = 0
        self._num_players = 3 # np.random.randint(2, 11)
  ...
  def is_simultaneous_node(self):
    return False # self._mode == 0 and self._cur_player < self._num_players - 1
  ...
  def observation_tensor(self):
    ...
    for player in range(10):
        obs.extend([0 for y in range(104)])
        if player < self._num_players:
            for card in self._bulls[player]:
                obs[-105 + card] = 1
  ...
  def action_to_string(self, player, action):
    if action < 105:
        return "card ({},{})".format(player, action)
