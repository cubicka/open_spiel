...
def _play_game(logger, game_num, game, bots, temperature, temperature_drop, fprint):
  ...
  while not state.is_terminal():
    ...
    for c in root.children:
        if not root_solved or state.is_chance_node():
...
def learner(*, game, config, actors, evaluators, broadcast_fn, logger):
    ...
    preview_model(logger, config, game, model, 0)
    ...
    def learn(step):
        ...
        preview_model(logger, config, game, model, step if step % config.checkpoint_freq == 0 else -1)
    ...
    for step in itertools.count(1):
        ...
        if (config.max_steps > 0 and step >= config.max_steps) or (latest_losses < config.learning_rate):
            ...
            preview_model(logger, config, game, model, step)
            break
...
def preview_model(logger, config, game, model, step):
...
def simulate(config, logger):
    ...
    # logger.print("\n", root.children_str(state))
    # logger.print("Player {} sampled action: {}".format(
    #     state.current_player(), state.action_to_string(state.current_player(), action)))
    # logger.print("With policy \n{}".format(policy))
    # logger.print()

    # rs = np.random.RandomState()
    # noise = rs.dirichlet([1] * 9)
    # legal_actions = [(1 - 0.25) * p + 0.25 * n
    #         for p, n in zip(policy, noise)]
    # logger.print("\n", noise)
    # logger.print("\n", legal_actions)

    # noise = rs.dirichlet([0.1] * 9)
    # legal_actions = [(1 - 0.25) * p + 0.25 * n
    #         for p, n in zip(policy, noise)]
    # logger.print("\n", noise)
    # logger.print("\n", legal_actions)
