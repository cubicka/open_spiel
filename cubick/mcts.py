...
class RandomRolloutEvaluator(Evaluator):
  ...
  def evaluate(self, state):
    ...
    for _ in range(self.n_rollouts):
      ...
      temp_returns = working_state.returns()[curr_player]
      returns = np.array([temp_returns, -1*temp_returns])
...
class MCTSBot(pyspiel.Bot):
  ...
  def _apply_tree_policy(self, root, state):
    ...
    """
    # print("## Apply Tree Policy")
    visit_path = [root]
    working_state = state.clone()
    current_node = root
    while not working_state.is_terminal() and (current_node.explore_count > 0 or working_state.is_chance_node()):
    #   print("From")
    #   print(current_node)
      ...
      if working_state.is_chance_node():
        ...
        # child_vals = [(c.explore_count and c.total_reward / c.explore_count)
        #     for c in current_node.children]
        # max_child = max(child_vals)
        # min_child = min(child_vals)
        # uct_c = max(self.uct_c, max_child - min_child)
  ...
  def mcts_search(self, state):
    ...
    opt_nums = len(state.legal_actions(state.current_player()))
    # print("MCTS Walk for", self.max_simulations)
    # for n in range(5):
      ...
      for node in reversed(visit_path):
        ...
        if solved and node.children:
          ...
          if player == pyspiel.PlayerId.CHANCE:
            ...
            best = [] if outcome is None else [0] * len(outcome)
            all_solved = True
            for child in node.children:
              if child.outcome is None:
                ...
                best = best + child.outcome

            if (all_solved):
              node.outcome = best / len(node.children)
            else:
              solved = False
          else:
